...
class Re3Tracker(object):
    def __init__(self, gpu_id=0, model_path=None):
        ...
        if model_path is not None:
            ...
            folder = os.path.join(LOG_DIR, "checkpoints_small" if USE_SMALL_NET else "checkpoints")
            pt_util.restore_from_folder(self.network, folder)
    ...
    def reset(self, unique_id=None):
        """
        Takip belleğini sıfırlar.
        Eğer unique_id verilirse sadece o nesneye ait takip geçmişi silinir.
        """
    ...
    def track(self, unique_id, image, starting_box=None):
        ...
        cv2.imwrite(f"debug_crop0_{unique_id}.jpg", cropped_input0[:, :, ::-1])
        cv2.imwrite(f"debug_crop1_{unique_id}.jpg", cropped_input1[:, :, ::-1])
